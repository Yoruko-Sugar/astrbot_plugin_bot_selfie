...
class BotSelfiePlugin(Star):
    ...
    async def _generate_selfie(self, event: AstrMessageEvent, outfit: str) -> str:
        ...
        if reference_image:
            prompt = f"把参考图片中的二次元人物形象改为一张自拍照片。视角1：拿着手机对着镜子拍照，视角2：拿着手机自拍时手机前置摄像头的视角，杜绝第三人称视角。请注意一定要保持参考图片中的风格，且自拍照片中的人物形象（脸部细节、身材细节）务必和参考图片中的形象保持一致，但不保留参考图片中人物形象的衣服内容，因为衣服内容会在下述内容中补充，仅保留参考图片中人物形象的脸部和身材即可。动作表情可以自由发挥。背景可以根据下述的详细内容自由发挥，但也应与参考图片风格一致（二次元、动漫画风），使整张图片的人物和背景比较协调。如下是详细的穿衣风格内容，请遵守上述规则，在仅改变人物动作和衣服风格的条件下进行改图：{outfit}"
        else:
            prompt = f"生成一张二次元角色的自拍照片，穿着：{outfit}。风格为二次元动漫风格，线条清晰，色彩鲜明，光线良好，背景简洁。"
        # 记录最终使用的参考图片路径
        if reference_image:
